...
def EG_MCTS_plan():
    ...
    routes = pickle.load(open(args.test_mols, 'rb'))
    # routes = []
    ...
    f = open(args.result_folder + f'/plan_0_{args.iterations}.pkl', 'wb')
    ...
    if args.collect_expe:
        for key in experience_filter.keys():
            ...
            if Q_list.count(-10.0) != 0:
                avg_Q = 0.0
            elif Q_list.count(10.0) != 0:
                avg_Q = 10.0
            else :
...
if __name__ == '__main__':
    ...
    setup_logger(f'plan_0_{args.iterations}.log')
